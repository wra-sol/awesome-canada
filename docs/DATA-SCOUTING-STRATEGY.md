...
- **WordPress REST API** — many mid-size Canadian city sites run WordPress (e.g., Weyburn). Query `https://[city].ca/wp-json/wp/v2/pages?per_page=100&_fields=slug,title,link` to enumerate all page slugs/titles when a JS-rendered site hides subpage links from curl. For agenda/minute posts, also check `/wp-json/wp/v2/categories` and `/wp-json/wp/v2/posts`.
